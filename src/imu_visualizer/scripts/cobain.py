...
class StarController:
    def __init__(self):
        rospy.init_node('star_controller')  # Ganti nama node

        # UDP Configuration
        self.udp_ip = "0.0.0.0"  # Listen to all interfaces
        self.udp_port = 4321  # Port untuk menerima data IMU
        ...
        self.command_pub = rospy.Publisher('/star_commands', String, queue_size=10)  # Ganti topik

        # ROS Subscriber
        rospy.Subscriber('/star_commands', String, self.command_callback)  # Ganti topik

        # Path message
        self.path_msg = Path()
        self.path_msg.header.frame_id = "map"

        rospy.loginfo("Star Controller Ready")
    ...
    def parse_udp(self, data):
        try:
            ...
            if decoded.startswith("ANGLE:"):
                ...
                moving = parts[1].split(':')[1] == "1" if len(parts) > 1 else False
                return yaw, moving
        except Exception as e:
